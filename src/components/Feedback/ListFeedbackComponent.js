...
class ListFeedbackComponent extends Component {
    ...
    componentDidUpdate(prevProps,prevState){
        if(prevState.feedbacks!==this.state.feedbacks && prevState.data1!==this.state.data1){
            const apiUrl = 'http://localhost:4000/api/AdminViewFeedback';
            fetch(apiUrl)
                .then(response => response.json())
                .then(data => {
        ...
        fetch(apiUrl, options)
            ...
            .then(
                (result) => {
                    ...
                    this.setState({
                        response: result,
                        data1: data1.filter(feedback => feedback.feeddback_id !== feedbackId)
                    })
    ...
    render() {
        ...
        return (
            <div className="container" >
                ...
                <div>
                    <FaSearch style={{ marginLeft:-35,marginTop:9,marginRight:9,color:"#000",float:"right",opacity:0.5}} />
                    <input
                            type="text"
                            className="input"
