...
import {FaSearch} from "react-icons/fa";
 ...
 class ListOrderDetails extends Component {
        ...
        fetch("http://localhost:4000/api/UpdateOrderData", requestOptions)
          ...
          .then(
            (result) => {
                ...
                this.setState({
                    response: result,
                    data1: data1.filter(order => order.placeorder_id !== placeorder_id)
                });
            },
            (error) => {
                this.setState({ error });
    ...
    componentDidMount() {
        ...
        fetch(apiUrl)
            .then(response => response.json())
            .then(data => this.setState({ data1: data.data }));          
    }
    componentDidUpdate(prevProps,prevState){
        if(prevState.order!==this.state.order && prevState.data1!==this.state.data1){
            const apiUrl = 'http://localhost:4000/api/ViewOrderData';
            fetch(apiUrl)
                .then(response => response.json())
                .then(data => {
    ...
    filterData(value) {
       // const excludeColumns = ["subservice_id", "image"];
        ...
        if (!lowerCaseValue) {
          this.setState({ data1: this.state.order });
        } else {
          const filteredData = this.state.order.filter((item) => {
